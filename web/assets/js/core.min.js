 ...
 */
var _typeof=typeof Symbol==='function'&&typeof Symbol.iterator==='symbol'?function(obj){return typeof obj}:function(obj){return obj&&typeof Symbol==='function'&&obj.constructor===Symbol&&obj!==Symbol.prototype?'symbol':typeof obj};(function(root){var setTimeoutFunc=setTimeout;function noop(){}function bind(fn,thisArg){return function(){fn.apply(thisArg,arguments)}}function Promise(fn){if(_typeof(this)!=='object')throw new TypeError('Promises must be constructed via new');if(typeof fn!=='function')throw new TypeError('not a function');this._state=0;this._handled=false;this._value=undefined;this._deferreds=[];doResolve(fn,this)}function handle(self,deferred){while(self._state===3){self=self._value}if(self._state===0){self._deferreds.push(deferred);return}self._handled=true;Promise._immediateFn(function(){var cb=self._state===1?deferred.onFulfilled:deferred.onRejected;if(cb===null){(self._state===1?resolve:reject)(deferred.promise,self._value);return}var ret;try{ret=cb(self._value)}catch(e){reject(deferred.promise,e);return}resolve(deferred.promise,ret)})}function resolve(self,newValue){try{if(newValue===self)throw new TypeError('A promise cannot be resolved with itself.');if(newValue&&((typeof newValue==='undefined'?'undefined':_typeof(newValue))==='object'||typeof newValue==='function')){var then=newValue.then;if(newValue instanceof Promise){self._state=3;self._value=newValue;finale(self);return}else if(typeof then==='function'){doResolve(bind(then,newValue),self);return}}self._state=1;self._value=newValue;finale(self)}catch(e){reject(self,e)}}function reject(self,newValue){self._state=2;self._value=newValue;finale(self)}function finale(self){if(self._state===2&&self._deferreds.length===0){Promise._immediateFn(function(){if(!self._handled){Promise._unhandledRejectionFn(self._value)}})}for(var i=0,len=self._deferreds.length;i<len;i++){handle(self,self._deferreds[i])}self._deferreds=null}function Handler(onFulfilled,onRejected,promise){this.onFulfilled=typeof onFulfilled==='function'?onFulfilled:null;this.onRejected=typeof onRejected==='function'?onRejected:null;this.promise=promise}function doResolve(fn,self){var done=false;try{fn(function(value){if(done)return;done=true;resolve(self,value)},function(reason){if(done)return;done=true;reject(self,reason)})}catch(ex){if(done)return;done=true;reject(self,ex)}}Promise.prototype['catch']=function(onRejected){return this.then(null,onRejected)};Promise.prototype.then=function(onFulfilled,onRejected){var prom=new this.constructor(noop);handle(this,new Handler(onFulfilled,onRejected,prom));return prom};Promise.all=function(arr){var args=Array.prototype.slice.call(arr);return new Promise(function(resolve,reject){if(args.length===0)return resolve([]);var remaining=args.length;function res(i,val){try{if(val&&((typeof val==='undefined'?'undefined':_typeof(val))==='object'||typeof val==='function')){var then=val.then;if(typeof then==='function'){then.call(val,function(val){res(i,val)},reject);return}}args[i]=val;if(--remaining===0){resolve(args)}}catch(ex){reject(ex)}}for(var i=0;i<args.length;i++){res(i,args[i])}})};Promise.resolve=function(value){if(value&&(typeof value==='undefined'?'undefined':_typeof(value))==='object'&&value.constructor===Promise){return value}return new Promise(function(resolve){resolve(value)})};Promise.reject=function(value){return new Promise(function(resolve,reject){reject(value)})};Promise.race=function(values){return new Promise(function(resolve,reject){for(var i=0,len=values.length;i<len;i++){values[i].then(resolve,reject)}})};Promise._immediateFn=typeof setImmediate==='function'&&function(fn){setImmediate(fn)}||function(fn){setTimeoutFunc(fn,0)};Promise._unhandledRejectionFn=function _unhandledRejectionFn(err){if(typeof console!=='undefined'&&console){console.warn('Possible Unhandled Promise Rejection:',err)}};Promise._setImmediateFn=function _setImmediateFn(fn){Promise._immediateFn=fn};Promise._setUnhandledRejectionFn=function _setUnhandledRejectionFn(fn){Promise._unhandledRejectionFn=fn};if(typeof module!=='undefined'&&module.exports){module.exports=Promise}else if(root!==undefined&&!root.Promise){root.Promise=Promise}if(!window.Promise){window.Promise=Promise}})(undefined);(function(){try{new CustomEvent('IE has CustomEvent, but doesn\'t support constructor')}catch(e){window.CustomEvent=function(event,params){var evt;params=params||{bubbles:false,cancelable:false,detail:undefined};evt=document.createEvent('CustomEvent');evt.initCustomEvent(event,params.bubbles,params.cancelable,params.detail);return evt};CustomEvent.prototype=Object.create(window.Event.prototype)}})();(function(arr){arr.forEach(function(item){if(item.hasOwnProperty('remove')){return}Object.defineProperty(item,'remove',{configurable:true,enumerable:true,writable:true,value:function remove(){this.parentNode.removeChild(this)}})})})([Element.prototype,CharacterData.prototype,DocumentType.prototype]);function themeSwitcher(options){'use strict';function Switcher(options){this.themes=options.themes;this.themeAttribute=options.themeAttribute||'data-theme-name';this.colorAttribute=options.colorAttribute||'data-color-picker';this.themeButtonSelector=options.themeButtonSelector||'['+this.themeAttribute+']';this.colorPickerSelector=options.colorPickerSelector||'['+this.colorAttribute+']';this.themeActiveClass=options.themeActiveClass||'active-theme';this.stylesLinkId=options.stylesLinkId||'main-styles-link';this.stylesNodeId=options.stylesNodeId||'main-styles-node';this.variablesFallback=false;this.regExp={sym003Bu:new RegExp('(url\\([^\\)]*?)(;)([^\\)]*?\\))','g'),sym007Bu:new RegExp('(url\\([^\\)]*?)({)([^\\)]*?\\))','g'),sym007Du:new RegExp('(url\\([^\\)]*?)(})([^\\)]*?\\))','g'),sym003Bq:new RegExp('(:\\s*?(\'|")[^\'"]*?)(;)([^\'"]*?(\'|")\\s*?;)','g'),sym007Bq:new RegExp('(:\\s*?(\'|")[^\'"]*?)({)([^\'"]*?(\'|")\\s*?;)','g'),sym007Dq:new RegExp('(:\\s*?(\'|")[^\'"]*?)(})([^\'"]*?(\'|")\\s*?;)','g'),wrap:new RegExp('(\\n|\\r)','g'),tab:new RegExp('\\t+?','g'),comment:new RegExp('\\/\\*(.|\\s)*?\\*\\/','g'),block:new RegExp('[^\\{\\};]*\\{(([^\\{\\};]*\\{(.|s)*?\\}\\s*?)|((.|s)*?))\\}','g'),selector:new RegExp('[^\\{\\};]*'),directive:new RegExp('@[^\\{\\};]*[^\\{]'),subblock:new RegExp('^[^\\{\\};]*?\\{(.*?)\\}[^\\{\\};]*?$'),rule:new RegExp('[^\\{\\};]+?:[^;]+?;','g'),properties:new RegExp('\\{(([^\\{\\};]+?:[^;]+?;\\s*?)*?)\\}')}}Switcher.prototype.hexToRGB=function(hex){var arr=hex.match(/[a-f0-9]{1,2}/ig);for(var i=0;i<arr.length;i++){arr[i]=parseInt(arr[i],16)}return arr};Switcher.prototype.getCookie=function(name){var matches=document.cookie.match(new RegExp('(?:^|; )'+name.replace(/([\.$?*|{}\(\)\[\]\\\/\+^])/g,'\\$1')+'=([^;]*)'));return matches?decodeURIComponent(matches[1]):undefined};Switcher.prototype.setCookie=function(name,value){value=encodeURIComponent(value);document.cookie=name+'='+value+'; path=/;'};Switcher.prototype.realType=function(object){return Object.prototype.toString.call(object)};Switcher.prototype.getFileContent=function(path){return new Promise(function(resolve,reject){var xhr=new XMLHttpRequest;xhr.open('GET',path,true);xhr.onload=function(){if(this.status==200){resolve(this.response)}else{throw new Error(this.statusText)}};xhr.onerror=function(){throw new Error('Network Error')};xhr.send(null)})};Switcher.prototype.initialStylesProcessing=function(string){return string.replace(this.regExp.comment,'').replace(this.regExp.wrap,' ').replace(this.regExp.tab,' ').replace(this.regExp.sym003Bu,'$1\\003B$3').replace(this.regExp.sym007Bu,'$1\\007B$3').replace(this.regExp.sym007Du,'$1\\007D$3').replace(this.regExp.sym003Bq,'$1\\003B$4').replace(this.regExp.sym007Bq,'$1\\007B$4').replace(this.regExp.sym007Dq,'$1\\007D$4').match(this.regExp.block)};Switcher.prototype.parseProperties=function(block){var rules=block.match(this.regExp.properties);if(rules)return rules[1].match(this.regExp.rule)};Switcher.prototype.parseBlocks=function(blocks){var arr=[];for(var i=0;i<blocks.length;i++){var directive=blocks[i].match(this.regExp.directive),subblock=blocks[i].match(this.regExp.subblock)[1].match(this.regExp.block);if(directive&&subblock){arr.push({type:'directive',value:directive[0],subblocks:this.parseBlocks(subblock)})}else{arr.push({type:'selector',value:blocks[i].match(this.regExp.selector)[0],properties:this.parseProperties(blocks[i])})}}return arr};Switcher.prototype.procColorReplace=function(data){if(this.realType(data)==='[object Array]'){for(var i=0;i<data.length;i++){if(data[i].type==='selector'){data[i].properties=this.procColorReplace(data[i].properties)}else if(data[i].type==='directive'){data[i].subblocks=this.procColorReplace(data[i].subblocks)}else{data[i]=this.procColorReplace(data[i])}}}else{if(data){var variables=this.themes[this.themeActive].variables;for(var key in variables){data=data.replace(new RegExp(variables[key],'gi'),'var('+key+','+variables[key]+')')}}}return data};Switcher.prototype.buildStyles=function(data){var stylesStr='';if(this.realType(data)==='[object Array]'){for(var i=0;i<data.length;i++){if(data[i].type==='selector'){stylesStr+=' '+data[i].value+' { '+this.buildStyles(data[i].properties)+' } '}else if(data[i].type==='directive'){stylesStr+=' '+data[i].value+' { '+this.buildStyles(data[i].subblocks)+' } '}else{stylesStr+=' '+this.buildStyles(data[i])+' '}}}else{stylesStr+=' '+data+' '}return stylesStr};Switcher.prototype.getActiveTheme=function(){var cookie=this.getCookie('themeSwitcher-active');if(cookie){this.themeActive=cookie}else{var stylesPath=document.getElementById(this.stylesLinkId).getAttribute('href');for(var key in this.themes){if(this.themes[key].stylesFile===stylesPath){this.setCookie('themeSwitcher-active',key);this.themeActive=key;break}}}};Switcher.prototype.getVariables=function(){var colorPickers=document.querySelectorAll(this.colorPickerSelector);for(var i=0;i<colorPickers.length;i++){var colorPicker=colorPickers[i],valueCookie=this.getCookie('themeSwitcher'+colorPicker.name),valueTheme=this.themes[this.themeActive].variables?this.themes[this.themeActive].variables[colorPicker.name]:false;if(valueCookie){colorPicker.value=valueCookie;document.documentElement.style.setProperty(colorPicker.name,valueCookie)}else if(valueTheme){colorPicker.value=valueTheme;document.documentElement.style.setProperty(colorPicker.name,valueTheme)}}};Switcher.prototype.includeInlineStyles=function(str){var stylesLink=document.getElementById(this.stylesLinkId),stylesNode=document.getElementById(this.stylesNodeId);this.newStyles=this.buildStyles(this.procColorReplace(this.parseBlocks(this.initialStylesProcessing(str)))).replace(/( ){2,}/g,' ');if(!stylesNode){var tmp=document.createElement('style');tmp.id=this.stylesNodeId;stylesNode=document.head.appendChild(tmp)}stylesNode.innerHTML=this.newStyles;if(stylesLink)stylesLink.remove()};Switcher.prototype.includeStylesFile=function(){var stylesLink=document.getElementById(this.stylesLinkId),stylesNode=document.getElementById(this.stylesNodeId);if(stylesLink)stylesLink.id=this.stylesLinkId+'-removing';var tmp=document.createElement('link');tmp.id=this.stylesLinkId;tmp.rel='stylesheet';tmp.href=this.themes[this.themeActive].stylesFile;var stylesLinkNew=document.head.appendChild(tmp);return new Promise(function(resolve,reject){stylesLinkNew.onload=function(){if(stylesNode)stylesNode.remove();if(stylesLink)stylesLink.remove();resolve()}})};Switcher.prototype.applyTheme=function(){this.throwEventSwitching();var switcher=this;if(this.themes[this.themeActive].variables){this.getFileContent(this.themes[this.themeActive].stylesFile).then(function(response){switcher.includeInlineStyles(response);switcher.throwEventSwitched()})}else{this.includeStylesFile().then(function(){switcher.throwEventSwitched()})}};Switcher.prototype.switchTheme=function(targetTheme){if(this.themeActive!==targetTheme){this.themeActive=targetTheme;this.applyTheme();this.setCookie('themeSwitcher-active',targetTheme)}};Switcher.prototype.throwEventSwitching=function(){var event=new CustomEvent('theme-switching',{cancelable:true,detail:this});document.documentElement.dispatchEvent(event)};Switcher.prototype.throwEventSwitched=function(){var event=new CustomEvent('theme-switched',{cancelable:true,detail:this});if(document.documentElement.dispatchEvent(event)){this.switchThemeButtonsClass()}};Switcher.prototype.switchThemeButtonsClass=function(){var themeButtonsDisable=document.querySelectorAll(this.themeButtonSelector);var themeButtonsEnable=document.querySelectorAll('['+this.themeAttribute+'="'+this.themeActive+'"]');for(var i=0;i<themeButtonsDisable.length;i++){themeButtonsDisable[i].classList.remove(this.themeActiveClass)}for(var i=0;i<themeButtonsEnable.length;i++){themeButtonsEnable[i].classList.add(this.themeActiveClass)}};Switcher.prototype.switchColor=function(variable,value){document.documentElement.style.setProperty(variable,value);this.setCookie('themeSwitcher'+variable,value)};var switcher=new Switcher(options),colorPicker=document.querySelectorAll(switcher.colorPickerSelector),themeButton=document.querySelectorAll(switcher.themeButtonSelector);switcher.getActiveTheme();switcher.getVariables();switcher.applyTheme();for(var i=0;i<colorPicker.length;i++){colorPicker[i].addEventListener('change',function(){switcher.switchColor(this.name,this.value)})}for(var i=0;i<themeButton.length;i++){themeButton[i].addEventListener('click',function(){switcher.switchTheme(this.getAttribute(switcher.themeAttribute))})}}